...
function getPolynom(...arg) {
  return arg.length === 0 ? null : (x) => arg
    .reduce((acc, el, index) => acc + (el * (x ** (arg.length - 1 - index))), 0);
}
...
function retry(func, attempts) {
  return function retFunc(n = attempts) {
    try {
      return func();
    } catch (err) {
      if (n !== 0) return retFunc(n - 1);
    }
    return undefined;
  };
}
...
function logger(func, logFunc) {
  return (...arg) => {
    const logStr = `${func.name}(${arg.map((el) => JSON.stringify(el)).join(',')})`;
...
function partialUsingArguments(fn, ...args1) {
  return function currFunc(...arg) {
    const args = args1.concat(arg);
